...
if (in_array($imageFileType, $extensions_arr)) {
    ...
    $query = "INSERT INTO `cases` (`id`, `client_id`, `lawyer_id`, `case_title`, `case_description`, `document`, `time`, `day`, `month`, `year`) VALUES (NULL, '$id', '$lawyerId', '$title', '$description', '$file','$time', '$day', '$month', '$year');";

    $data = mysqli_query($connect, "$query");

    if ($data) {
        move_uploaded_file($_FILES['file']['tmp_name'], $target_dir . $fileName);
        echo "<script>alert('Your Case Submitted well done');
    window.open('index.php','_self')</script>";
    } else {
        echo "ERROR: " . mysqli_error($connect);
    }
} else {
    echo "<script>alert('please select pdf file');
    history.go(-1)</script>";
}
