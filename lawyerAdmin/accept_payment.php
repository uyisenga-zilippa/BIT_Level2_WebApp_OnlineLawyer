...
$query = "UPDATE `applications` SET `isPayed`='yes' WHERE `id`='$id'";
$done = mysqli_query($connect, "$query");
$paymentQuery = "INSERT INTO `payments` (`id`, `school_id`, `student_id`, `amount`, `day`, `month`, `year`) VALUES ('', '$schoolId', '$stId', '$amount[0]', '$day', '$month', '$year');";
$studentInfor = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM `student` WHERE `id`='$stId'"));

$sendTo = $studentInfor[3];
$studentName = "$studentInfor[1] $studentInfor[2]";
$msg = " Hello $studentName<br>your payment of <b>$amount[0] Frw</b> has approved";

$schoolInfor = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM `schools` WHERE `id`='$schoolId'"));
$sendFrom = $schoolInfor['email'];
$schoolName = $schoolInfor['name'];
